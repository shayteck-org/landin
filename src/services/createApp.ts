...
export default async function createApp(values: { app: string, domain: string, aiprompt: string | undefined }) {
  setTokenForAPI();

  try {
    const response = await http.post(addresses.createApp, { name: values.app, domain: values.domain, aiprompt: values.aiprompt });
